...
import { Plus, LayoutGrid, LogOut, Radar, Search, Settings, ShieldAlert, UserRoundPen, Zap } from 'lucide-react';
    ...
    return (
        <aside className="w-[240px] flex-shrink-0 border-r border-thin border-border-color bg-surface flex flex-col h-full z-10 hidden md:flex">
            ...
            <div className="flex flex-col py-4 px-2 space-y-1">
                ...
                <Link
                    to="/hunts"
                    className={`flex items-center gap-3 px-3 py-2 text-sm font-medium transition-colors border-l-2 ${location.pathname === '/hunts' ? 'bg-row-hover text-text-main border-primary' : 'text-muted border-transparent hover:bg-row-hover'}`}
                >
